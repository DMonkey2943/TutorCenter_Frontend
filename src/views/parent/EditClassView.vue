<template>
  <div class="container-xxl py-5">
    <div class="container">
      ...
      >
        <h1 class="text-center mb-2">Chỉnh sửa lớp học</h1>
...
const getClass = async () => {
  try {
    ...
    if (result.data.status !== 0) {
      message.error("Bạn không thể chỉnh sửa lớp học được nữa!");
      router.push({
        name: "parent.classes.detail",
        params: { id }, // Chuyển hướng với ID lớp học
      });
...
const updateClass = async () => {
  try {
    ...
    if (result.success) {
      ...
      router.push({ name: "parent.classes.detail", params: { id: classId } });
    }
  } catch (error) {
    console.error(error);
